...
> - `canvas-pipeline.png` - ReactFlow canvas with 5-agent pipeline, all nodes green
> - `fleet-dashboard.png` - Fleet dashboard with live GPU market pricing
> - `mission-output.png` - Output panel with full competitive analysis report
> - `chat-narration.png` - Chat panel with real-time mission narration
...
| POST | `/fleet/mission/execute` | Start a mission (body: `{mission: string}`) |
...
# OPENAI_BASE_URL and OPENAI_API_KEY are pre-configured for Nosana inference
...
frontend/src/
  ...
  lib/
    ...
    markdown.ts                               # Shared markdown-to-HTML renderer
...
|----------|----------|-------------|
| `NOSANA_API_KEY` | For live deployments | Nosana platform API key. Empty = mock mode |
| `OPENAI_API_KEY` | Yes | API key for LLM inference endpoint |
| `OPENAI_BASE_URL` | Yes | LLM inference base URL (Nosana or Ollama) |
...
| `MODEL_NAME` | No | Legacy model name, passed to worker containers |
| `OPENAI_API_URL` | No | Legacy URL, passed to worker containers |
| `TAVILY_API_KEY` | For web search | Tavily API key for researcher agents |
| `AGENTFORGE_WORKER_IMAGE` | No | Docker image for worker agents. Default: `drewdockerus/agentforge-worker:latest` |
| `FLEET_API_PORT` | No | Fleet API port. Default: `3001` |
| `SERVER_PORT` | No | ElizaOS server port. Default: `3000` |
...
- **Stack:** ElizaOS v2 (1.7.2) + Nosana GPU Network (@nosana/kit 2.2.4) + React 19 + ReactFlow + Zustand + Tailwind 4 + shadcn/ui
