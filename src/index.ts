...
if (process.env.AGENT_TEMPLATE) {
  // ─── Worker Mode ───────────────────────────────────────────────────────────
  // When AGENT_TEMPLATE is set, this image runs as a worker on Nosana.
  // Creates a dynamic ElizaOS agent from environment variables.

  const AGENT_NAME = process.env.AGENT_NAME || 'Worker Agent';
  const AGENT_TEMPLATE = process.env.AGENT_TEMPLATE!;
  ...
  // ─── Orchestrator Mode ─────────────────────────────────────────────────────
  // Default: load forge-master character + nosana plugin
  const { nosanaPlugin } = await import('./plugins/nosana/index.js');
